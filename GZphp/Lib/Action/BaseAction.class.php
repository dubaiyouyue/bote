...
//print_r($_GET);exit;
		  ...
		  $lanmuffwz=array_merge($lanmuffwz,$lanmuzx);
			...
			//new start
			//所有栏目
			global $new_column_list,$location_two_id;
				...
				foreach($new_column_list as $nc=>$nk){
					if($nk['id']==$cid){
						...
						if($nk['classtype']==3){
							...
							$location_two_enname=$location_two_cinfo['enname'];
							//一级栏目
						...
						$this->assign('location_one_id',$location_one_id);
						$this->assign('location_two_id',$location_two_id);
						$this->assign('location_two_name',$location_two_name);
						$this->assign('location_two_enname',$location_two_enname);
				...
				if($now_cid_first){
					foreach($new_column_list as $nnc=>$nnk){
						if($nnk['bigclass']==$now_cid_first){
							...
							$now_cid_cfirst_foldername=$nnk['foldername'];
					...
					if($now_cid_cfirst && !$namemark && ($now_cid_first==$cid)){
						header('Location:/index.php/Home/'.$now_cid_cfirst_foldername.'/index/cid/'.$now_cid_cfirst.'.html');
						exit;
					};
			...
			if(!$cid){
				...
				$enseo_title='HOME-'.$new_config_info['engz_webname'];
				$seo_keywords=$new_config_info['gz_keywords'];
				$seo_description=$new_config_info['gz_description'];
			}else{
				$wcinfo['id']=$cid;
				$cinfo=M('column')->where($wcinfo)->field('enname,name,keywords,description,ctitle')->find();
				$seo_title=$cinfo['name'].'-'.$new_config_info['gz_webname'];
				$enseo_title=$cinfo['enname'].'-'.$new_config_info['engz_webname'];
				if($cinfo['ctitle']) $seo_title=$cinfo['ctitle'].'-'.$new_config_info['gz_webname'];
				if($cinfo['ctitle']) $enseo_title=$cinfo['ctitle'].'-'.$new_config_info['engz_webname'];
			...
			//print_r($id);exit;
				...
				//print_r($newssinfo);exit();
					...
					$enseo_title=$newssinfo['ctitle'].'-'.$enseo_title;
					if($cinfo['ctitle']) $seo_title=$newssinfo['ctitle'].'-'.$seo_title;
					if($cinfo['ctitle']) $enseo_title=$newssinfo['ctitle'].'-'.$enseo_title;
			...
			$this->assign('enseo_title',$enseo_title);
