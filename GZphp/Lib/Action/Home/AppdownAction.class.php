...
class AppdownAction extends BaseAction
{
    public function index()
    {
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(511);
		$getinfolamu=$this->get_column_info(511);$this->assign('getinfolamu',$getinfolamu);
		//dump($getinfolamu);exit;
		   $lanmu=M('column')->where('bigclass=511')->order('no_order asc,id asc')->select();
		
		  /*  $news=M('news')->where("class1=511")->select();
