...
class PlatformAction extends BaseAction
{
    public function index()
    {
		...
		global $foldername;
		global $namemark_tpl;
		$blist=$this->bannerlists(508);
		$getinfolamu=$this->get_column_info(508);$this->assign('getinfolamu',$getinfolamu);
		//dump($getinfolamu);exit;
		   $lanmu=M('column')->where('bigclass=508')->order('no_order asc,id asc')->select();
		
		  /*  $news=M('news')->where("class1=508")->select();
