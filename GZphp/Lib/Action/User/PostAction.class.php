...
class PostAction extends BaseAction {
	...
	public function Message() {
        ...
        if(empty($_POST['tel']) || empty($_POST['name']) || empty($_POST['email']) || empty($_POST['guojia']) || empty($_POST['telm']) || empty($_POST['cxsxx']) || empty($_POST['message'])) {
            $this->error(L('信息填写不完整!'));
            exit;
        }
		$_POST['tel']=$_POST['telm'].' '.$_POST['tel'];
		$_POST['message']=$_POST['cxsxx'].' '.$_POST['message'];
		$_POST['name']=$_POST['guojia'].' '.$_POST['name'];
   ...
   $this->fields['email'] = htmlspecialchars(trim($this->fields['email']));
    ...
    $this->fields['message'] = htmlspecialchars(trim($this->fields['message']));
			...
			if($_GET['ajax']=='ajax'){
				...
				if(M('message')->add($ajax_date)) echo 'addok';
				return false;
			}
        if ($this->moduleid == 0) {
