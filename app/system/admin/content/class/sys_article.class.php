...
class sys_article extends module {
	...
	public function update_list_sql($list,$id){
		...
		if($list['endescription']){
			$query = "SELECT encontent FROM {$this->tablename} WHERE id='{$id}'";
			$listown = DB::get_one($query); 
			$endescription = $this->description($listown['encontent']);
			if($list['endescription']==$description){
				$list['endescription'] = $this->description($list['encontent']);
			}
		}else{
			$list['endescription'] = $this->description($list['encontent']);
		}
		...
		$query = "UPDATE {$this->tablename} SET
			...
			endescription        = '{$list['endescription']}',
			description        = '{$list['description']}',
			content            = '{$list['content']}',
			encontent            = '{$list['encontent']}',
			class1             = '{$list['class1']}',
			class2             = '{$list['class2']}',
			class3             = '{$list['class3']}',
			imgurl             = '{$list['imgurl']}',
	...
	public function insert_list_sql($list){
		...
		if(!$list['endescription'])$list['endescription'] = $this->description($list['encontent']);
